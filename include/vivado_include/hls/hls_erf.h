...
T generic_erf(T x)
{
    ...
    if(FP_SUBNORMAL == ::hlstmp::fpclassify(x)) {
        return 0;
    }
	...
	if(ix < erf_traits<T>::segment[2]) {		/* |x|<0.84375 */
	    ...
	    s = coeff<T>::one+z*(qq1+z*(qq2+z*(qq3+z*(qq4+z*qq5))));
#else
	...
	if(ix < erf_traits<T>::segment[5]) {		/* 0.84375 <= |x| < 1.25 */
	    ...
	    Q = coeff<T>::one+s*(qa1+s*(qa2+s*(qa3+s*(qa4+s*(qa5+s*qa6)))));
#else
	...
	if(ix< erf_traits<T>::segment[7]) {	/* |x| < 1/0.35 */
	    ...
	    R=ra0+s*(ra1+s*(ra2+s*(ra3+s*(ra4+s*(
				ra5+s*(ra6+s*ra7))))));
	    S=coeff<T>::one+s*(sa1+s*(sa2+s*(sa3+s*(sa4+s*(
	    ...
	    R=rb0+s*(rb1+s*(rb2+s*(rb3+s*(rb4+s*(
				rb5+s*rb6)))));
	    S=coeff<T>::one+s*(sb1+s*(sb2+s*(sb3+s*(sb4+s*(
				sb5+s*(sb6+s*sb7))))));
#else
	...
	r  =  ::hlstmp::exp(-z*z-(T)0.5625)*::hlstmp::exp((z-x)*(z+x)+R/S);
	//r  =  ::hlstmp::exp(-x*x-(T)0.5625+R/S); // also work 
...
T generic_erfc(T x)
{
	...
	if (ix < erfc_traits<T>::segment[6]) {		/* |x|<28 */
	    ...
	    r  =  ::hlstmp::exp(-z*z-(T)0.5625)*
			::hlstmp::exp((z-x)*(z+x)+R/S);
        T r_x = r/x;
	    if(hx>0) {
            if(FP_SUBNORMAL == ::hlstmp::fpclassify(r_x)) {
                return 0;
            }
            return r_x; 
        } else 
