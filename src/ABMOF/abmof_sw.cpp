...
//	cout << "Data after write : " << slicesSW[sliceIdx][x][y/COMBINED_PIXELS].range(4 * yNewIdx + 3, 4 * yNewIdx) << endl;
    ...
    // write scale 1
    ap_uint<10> xScale1 = x/2;
    ap_uint<10> yScale1 = y/2;
...
//	cout << "Data after write : " << slicesScale1SW[sliceIdx][xScale1][yScale1/COMBINED_PIXELS].range(4 * yNewIdxScale1 + 3, 4 * yNewIdxScale1) << endl;
    ...
    // write scale 2
    ap_uint<10> xScale2 = x/4;
    ap_uint<10> yScale2 = y/4;
...
//        cout << i << ":\t" << ts << "\t" << xWr << "\t" << yWr << "\t" << pol << endl;
//        cout << BLOCK_SIZE_SCALE_0 << "\t" << BLOCK_SIZE_SCALE_1 << "\t" << BLOCK_SIZE_SCALE_2 << endl;
        ...
        if( areaCountExceeded || (ts - currentTs) >= MAX_SLICE_DURATION_US )
        {
            ...
            for(int r = 0; r < 1; r++)
            {
                ...
                cout << "deltaTs is: " << ((currentTs - lastTs) >> 9) << endl;
            }
...
//        ap_uint<32> output = (ap_uint<32>(yWr) << 16) + (xWr << 1) + pol;
        ap_uint<32> output = 0;
        output.range(9, 0) = xWr;
        output.range(19, 10) = yWr;
